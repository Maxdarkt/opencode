...
3. `git push origin staging` (créer la branche distante si absente). Ne pas pousser vers `dev` / `develop` / `main` / `master` **par cette porte candidate**. Jamais `--force`. `origin/dev` = OpenCode amont, hors palier Daidalon.
4. `git worktree remove` seulement les worktrees `features/tasks/` du sprint clos, **propres**. Sprint 8 : 014, 015, 007 puis 020. Mandat 2026-09-25 : 7 `s3-*` propres retirés ; APEX uniques copiés sur `staging` avant `--force` des 9 `10-*`…`s2-*` sales.
5. Checkpoint après `staging` sain, **mandat explicite** : créer ou avancer `develop` (preview) et `master` (prod) **depuis `staging`**. Sprint 8 : premier `develop` / `master` = même SHA que `staging`. Jamais `origin/dev`. Jamais `--force`.
