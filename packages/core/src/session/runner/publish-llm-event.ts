...
export const createLLMEventPublisher = (events: EventV2.Interface, input: Input) => {
  ...
  let stepSettlement:
    | { readonly finish: string; readonly tokens: ReturnType<typeof tokens>; readonly measured: boolean }
    | undefined
  ...
  const publish = Effect.fn("SessionRunner.publishLLMEvent")(function* (
    ...
    switch (event.type) {
      ...
      case "step-finish":
        ...
        stepSettlement = { finish: event.reason, tokens: tokens(event.usage), measured: event.usage !== undefined }
        return
      case "finish":
        return
      case "provider-error":
